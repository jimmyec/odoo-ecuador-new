...
class account_einvoice_wizard(models.TransientModel):
	...
	def action_automatic_einvoice(self):
		...
		Metodo ...
		"""
		invoices = self.env['account.invoice'].search([('autorizado_sri','=',False),('type','=','out_invoice')])   
	...
	def action_automatic_send_email(self):
		...
		Metodo ...
		"""
		invoices = self.env['account.invoice'].search([('to_send_einvoice','=',True),('type','=','out_invoice')])   
